...
> **Quick answer before you read further:** if you're forking this to research, teach, or run exploratory simulations, you need nothing beyond what's already in this repo — go fork it. If you're moving toward a real-world pilot, see [`GOVERNANCE_FAQ_v1.0.md`](./GOVERNANCE_FAQ_v1.0.md) and the Pre-Deployment Checklist below.
...
*	A clean conceptual "core" designed for reuse — formalized as machine-readable ontology files in `knowledge-engineering/` (see below)
*	A companion evidence document (MECHANISM_EVIDENCE.md) reviewing the historical record for PAPO's underlying opportunity-vs-alert mechanism
...
________________________________________
## Mechanism Evidence: Has This Kind of Thing Ever Worked?

PAPO has no field pilot yet, so this repo cannot point to PAPO deployment data. What it can point to is decades of real heat-emergency history that already tested PAPO's underlying idea — personalized, feasible opportunity beats generic alerts — without calling it that.

**MECHANISM_EVIDENCE.md** (added 2026-08-11) reviews three independent, peer-reviewed or extensively documented historical cases supporting this mechanism — the 1995 Englewood vs. Auburn Gresham mortality gap, Milwaukee's 1995-vs-1999 outcome improvement after adding targeted outreach, and the Naughton et al. individual-level case-control study — alongside one honest complication (a Duneier reanalysis) that keeps the claim from being overstated.

This evidence is about the mechanism, not this specific AI-driven implementation. It does not validate PAPO's architecture, predict effect sizes for any given city, or substitute for the Pre-Deployment Checklist below. Read it before forking if you want to know whether the underlying idea has ever actually held up outside a simulation.

________________________________________
## Knowledge Engineering: Formal Ontology (`knowledge-engineering/`)

This repository includes a formal, machine-readable ontology of the PAPO–Heatwave framework, built as a two-layer OWL-style schema in YAML plus an accompanying flowchart:

* **`papo_core_ontology.yaml`** — the hazard-agnostic PAPO middle-range theory itself: classes, object properties, data properties, and disjointness/restriction axioms covering `Policy`, `Opportunity`, `PopulationSegment`, `Resource`, the four-component system architecture (Policy Input Layer, Personalized Opportunity Engine, Opportunity Delivery Interface, Feedback Loop Module), and more. Contains no heatwave-specific vocabulary, so it is reusable for future hazard applications (wildfire smoke, flooding, cold snap, etc.).
* **`papo_heatwave_extension_ontology.yaml`** — the heat-specific delta only (heat alerts, cooling-center resources, heat scenario context, etc.), each traced to a specific CMDDS Step, SSRN Section, or textbook Table for provenance. Imports the core file; not a standalone schema on its own.
* **`papo_core_flowchart.md`** — a Mermaid flowchart rendering the hazard-agnostic translation/state-transition logic (Step 3's "non-negotiable logic core") as a diagram, with edge labels traceable 1:1 to the core ontology's object properties. Renders natively on GitHub.

These files are TBox-only (schema-level): no cities, no named facilities, no instances. They are meant to be the stable conceptual layer that per-city or per-fork implementations (Cypher/Neo4j, RDF/OWL, or plain code) instantiate against — consistent with this repo's own "implementation-agnostic" design philosophy above. See each file's internal `designNotes` for the reasoning behind specific modeling choices, including where content drawn from the SSRN paper and the Springer textbook chapter was judged hazard-agnostic (→ core) versus heatwave-specific (→ extension).
...
> See MECHANISM_EVIDENCE.md for the historical rationale behind the "Opportunity uptake vs. alert-only baselines" and "Equity impact comparison across neighborhoods" projects — they are the closest local analogues to the strongest historical evidence for PAPO's core mechanism.
...
- **★ MOKG-PH Case Report:** Wu, M. (April 2026). *PAPO-Heatwave MOKG-PH Case Analysis Report* (Case #002-C). MOKG-PH framework analysis that triggered this v1.2 update; identifies five structural gaps in MOKG-PH v1.1 (resolved in MOKG-PH v1.2) and two pre-deployment resolutions for PAPO forks. (internal case analysis, unpublished)
- **Mechanism Evidence:** MECHANISM_EVIDENCE.md (this repo, added 2026-08-11) — historical evidence review for PAPO's core opportunity-vs-alert mechanism, drawing on Klinenberg (2002), Weisskopf et al. (2002), Naughton et al. (2002), Browning et al. (2006), and Duneier (2006).
- **Formal Ontology:** `knowledge-engineering/papo_core_ontology.yaml`, `knowledge-engineering/papo_heatwave_extension_ontology.yaml`, `knowledge-engineering/papo_core_flowchart.md` (this repo, added 2026-08-19) — machine-readable TBox schema and flowchart formalizing the PAPO framework, cross-checked against the CMDDS, the SSRN preprint, and the Springer textbook chapter (Section 8.7).
- **Governance FAQ:** `GOVERNANCE_FAQ_v1.0.md` (this repo, added 2026-08-20) — plain-language index distinguishing research/simulation use (no gate) from real-world pilot use (Pre-Deployment Checklist applies); not a substitute for the CMDDS or for legal advice.
...
- **Documentation addendum (2026-08-11)** — Added MECHANISM_EVIDENCE.md, a companion document reviewing the historical evidence base for PAPO's core opportunity-vs-alert mechanism, and cross-referenced it from the Overview, Mechanism Evidence, Suggested Starter Simulation Projects, and Related References sections. Does not alter the CMDDS v1.2 specification content; version number unchanged.
- **Documentation addendum (2026-08-19)** — Added `knowledge-engineering/`, containing a formal two-layer ontology (`papo_core_ontology.yaml`, hazard-agnostic; `papo_heatwave_extension_ontology.yaml`, heat-specific delta) and an accompanying flowchart (`papo_core_flowchart.md`) rendering the Step 3 translation logic. Cross-checked against the CMDDS, the SSRN preprint, and Section 8.7 of the Springer textbook chapter. Cross-referenced from "What This Repository Is" and Related References. Does not alter the CMDDS v1.2 specification content; version number unchanged.
- **Documentation addendum (2026-08-20)** — Added `GOVERNANCE_FAQ_v1.0.md`, a plain-language FAQ routing research/simulation forks (no gate) from real-world pilot forks (Pre-Deployment Checklist applies). Cross-referenced from a new callout near the top of this README and from Related References. Does not alter the CMDDS v1.2 specification content; version number unchanged.
